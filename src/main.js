    ...
    <main>
      <aside class="sidebar">
        ...
        <section class="coordinate-card">
          ...
          <div class="background-settings"><div class="card-title"><span>▧</span><div><b>พื้นหลังภูมิศาสตร์</b><small>PROJECT + REGIONAL TERRAIN</small></div></div><label class="crs-field">ระบบพิกัดโครงการ<select id="projectCrs"><option value="EPSG:3123" selected>PRS92 / Philippines Zone 3 (EPSG:3123)</option><option value="UTM">WGS84 / UTM</option></select></label><div class="background-grid" id="utmSettings" hidden><label>UTM Zone<input id="utmZone" type="number" min="1" max="60" value="51"></label><label>Hemisphere<select id="utmHemisphere"><option value="N" selected>North</option><option value="S">South</option></select></label></div><div class="regional-terrain"><label>วิวภูมิประเทศรอบโครงการ<select id="regionalRadius"><option value="2000">รัศมี 2 km · ละเอียด</option><option value="5000" selected>รัศมี 5 km · แนะนำ</option><option value="10000">รัศมี 10 km · ภาพรวม</option></select></label><button class="secondary" id="loadRegionalTerrain">โหลด Terrain รอบโครงการ</button><label class="toggle-row">แสดง Regional Terrain<input id="regionalVisible" type="checkbox" checked><i></i></label><div class="layer-offset"><label>Regional Terrain Offset (m)<input id="regionalOffset" type="number" step="0.001" value="0"></label><button class="secondary" id="applyRegionalOffset">ใช้</button><div class="offset-nudge"><button data-offset-target="regional" data-offset-step="-1">−1</button><button data-offset-target="regional" data-offset-step="-.1">−0.1</button><button data-offset-target="regional" data-offset-step=".1">+0.1</button><button data-offset-target="regional" data-offset-step="1">+1</button></div><small id="regionalOffsetStatus">Offset +0.000 m · Blend ที่ขอบ Project DEM</small></div><small id="regionalStatus">Mapzen Terrain + Esri Imagery · ยังไม่ได้โหลด</small></div><div class="background-grid"><label>รัศมี OSM<select id="osmRadius"><option value="300">300 m</option><option value="600" selected>600 m</option><option value="1000">1,000 m</option></select></label><label>แหล่งภาพโครงการ<select id="imageryProvider"><option value="esri" selected>Esri World Imagery</option><option value="maptiler">MapTiler Satellite</option></select></label><label>ขอบเขตภาพ<select id="imageryRadius"><option value="100">100 m · เหมาะกับ Zoom 19–20</option><option value="250" selected>250 m · แนะนำ</option><option value="600">600 m · ภาพรวม</option></select></label><label>ความละเอียด (Zoom)<select id="imageryZoom"><option value="14">14 · ประหยัดข้อมูล</option><option value="15">15</option><option value="16">16</option><option value="17" selected>17 · แนะนำ</option><option value="18">18 · ละเอียด</option><option value="19">19 · ละเอียดมาก</option><option value="20">20 · สูงสุดตามพื้นที่</option></select></label></div><label class="imagery-key" id="mapTilerKeyField" hidden>MapTiler Public API Key<input id="mapTilerKey" type="password" autocomplete="off" placeholder="ไม่บันทึกลง GitHub"><small>ใช้เฉพาะในหน่วยความจำของหน้าเว็บนี้</small></label><small id="imageryResolutionHint" class="imagery-resolution-hint">Zoom 17 · ประมาณ 1.17 m/px ที่เส้นศูนย์สูตร</small><label class="dem-picker">Project DEM GeoTIFF<input id="demInput" type="file" accept=".tif,.tiff"><span id="demStatus">ยังไม่ได้โหลด Project DEM</span></label><div class="project-dem-lock"><b>Project DEM เชื่อมกับ Point Cloud</b><span>Offset ปกติ 0.000 m · Esri Imagery ติดตามผิว DEM</span></div><details class="advanced-calibration"><summary>เครื่องมือแก้ระดับ Project DEM ขั้นสูง</summary><div class="vertical-calibration"><label>DEM Vertical Offset (m)<input id="demVerticalOffset" type="number" step="0.001" value="0"></label><button class="secondary" id="applyVerticalOffset">ใช้ Offset</button><label>ระดับจริงของ Control Point (m)<input id="controlElevation" type="number" step="0.001" placeholder="เช่น 102.450"></label><button class="secondary" id="calibrateTerrain">เลือกจุดบน DEM เพื่อปรับระดับ</button><small id="verticalStatus">Corrected Z = DEM Z + 0.000 m</small></div></details><label class="toggle-row background-toggle">แสดง Project Terrain<input id="terrainVisible" type="checkbox" checked><i></i></label><button class="secondary imagery-button" id="loadImagery">โหลดภาพโครงการ</button><label class="toggle-row background-toggle">แสดงภาพโครงการ<input id="imageryVisible" type="checkbox" checked><i></i></label><div class="layer-offset imagery-offset"><label>Project Esri Imagery Offset (m)<input id="imageryOffset" type="number" step="0.001" value="0"></label><button class="secondary" id="applyImageryOffset">ใช้</button><div class="offset-nudge"><button data-offset-target="imagery" data-offset-step="-1">−1</button><button data-offset-target="imagery" data-offset-step="-.1">−0.1</button><button data-offset-target="imagery" data-offset-step=".1">+0.1</button><button data-offset-target="imagery" data-offset-step="1">+1</button></div><small id="imageryOffsetStatus">Offset +0.000 m · อ้างอิงผิว Project DEM</small></div><button class="secondary" id="loadBackground">โหลดอาคารและถนน OSM</button><div class="osm-offsets"><div class="layer-offset"><label>OSM Buildings Offset (m)<input id="osmBuildingsOffset" type="number" step="0.001" value="0"></label><button class="secondary" id="applyOsmBuildingsOffset">ใช้</button><div class="offset-nudge"><button data-offset-target="buildings" data-offset-step="-1">−1</button><button data-offset-target="buildings" data-offset-step="-.1">−0.1</button><button data-offset-target="buildings" data-offset-step=".1">+0.1</button><button data-offset-target="buildings" data-offset-step="1">+1</button></div><small id="osmBuildingsOffsetStatus">Offset +0.000 m</small></div><div class="layer-offset"><label>OSM Roads Offset (m)<input id="osmRoadsOffset" type="number" step="0.001" value="0"></label><button class="secondary" id="applyOsmRoadsOffset">ใช้</button><div class="offset-nudge"><button data-offset-target="roads" data-offset-step="-1">−1</button><button data-offset-target="roads" data-offset-step="-.1">−0.1</button><button data-offset-target="roads" data-offset-step=".1">+0.1</button><button data-offset-target="roads" data-offset-step="1">+1</button></div><small id="osmRoadsOffsetStatus">Offset +0.000 m</small></div></div><label class="toggle-row background-toggle">แสดง OSM 3D<input id="backgroundVisible" type="checkbox" checked><i></i></label><small class="osm-credit">Terrain © Mapzen contributors · <span id="imageryCredit">Imagery © Esri and data providers</span> · <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap contributors</a></small></div>
...
const regionalTerrainGroup=new THREE.Group();regionalTerrainGroup.name='Mapzen regional terrain';scene.add(regionalTerrainGroup);
...
const osmGroundGroup=new THREE.Group(),osmBuildingsGroup=new THREE.Group(),osmRoadsGroup=new THREE.Group();osmGroundGroup.name='OSM ground';osmBuildingsGroup.name='OSM buildings';osmRoadsGroup.name='OSM roads';backgroundGroup.add(osmGroundGroup,osmBuildingsGroup,osmRoadsGroup);
let terrainData=null,terrainVerticalOffset=0,regionalVerticalOffset=0,imageryVerticalOffset=0,osmBuildingsOffset=0,osmRoadsOffset=0;

const state = { layers: [], selectedId: null, selectedEntity:null, origin: new THREE.Vector3(), nextId: 1, measuring:false, measureStart:null, measurePreview:null, measureAxis:'auto', measureTarget:'surface', snapPoint:null, pivotMode:false, calibratingTerrain:false };
...
function hasOsmBackground(){return osmGroundGroup.children.length+osmBuildingsGroup.children.length+osmRoadsGroup.children.length>0}
$('#projectCrs').addEventListener('change',e=>{const utm=e.target.value==='UTM';$('#utmSettings').hidden=!utm;if(hasOsmBackground()||imageryGroup.children.length||terrainGroup.children.length||regionalTerrainGroup.children.length){clearBackground();clearImagery();clearTerrain();clearRegionalTerrain()}toast(`เปลี่ยนระบบพิกัดเป็น ${utm?'WGS84 / UTM':'PRS92 Zone 3 (EPSG:3123)'} · กรุณาโหลดพื้นหลังใหม่`)});
function clearTerrain(){terrainGroup.traverse(o=>{o.geometry?.dispose();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>m.dispose())});terrainGroup.clear();terrainData=null;$('#demStatus').textContent='ยังไม่ได้โหลด Project DEM';invalidate()}
...
function terrainHeightAt(x,y){
  ...
  const fx=(x-bounds.minX)/(bounds.maxX-bounds.minX)*(width-1),fy=(bounds.maxY-y)/(bounds.maxY-bounds.minY)*(height-1),x0=Math.floor(fx),y0=Math.floor(fy),x1=Math.min(x0+1,width-1),y1=Math.min(y0+1,height-1),tx=fx-x0,ty=fy-y0,v00=values[y0*width+x0],v10=values[y0*width+x1],v01=values[y1*width+x0],v11=values[y1*width+x1];if(![v00,v10,v01,v11].every(v=>validElevation(v,noData)))return null;return THREE.MathUtils.lerp(THREE.MathUtils.lerp(v00,v10,tx),THREE.MathUtils.lerp(v01,v11,tx),ty)-state.origin.z+terrainVerticalOffset;
}
...
async function loadDem(file){
  ...
  if(!(bounds.maxX>bounds.minX&&bounds.maxY>bounds.minY))throw new Error('ขอบเขตพิกัด GeoTIFF ไม่ถูกต้อง');const noDataRaw=image.getGDALNoData(),parsedNoData=noDataRaw==null?null:Number(noDataRaw),noData=Number.isFinite(parsedNoData)?parsedNoData:null,geometry=makeTerrainGeometry(bounds,width,height,values,noData),material=new THREE.MeshStandardMaterial({color:0x49655c,roughness:1,metalness:0,side:THREE.DoubleSide,transparent:true,opacity:.9}),mesh=new THREE.Mesh(geometry,material),hadImagery=imageryGroup.children.length>0,hadRegional=regionalTerrainGroup.children.length>0;mesh.userData.contextOnly=true;clearTerrain();if(hadImagery)clearImagery();if(hadRegional)clearRegionalTerrain();terrainData={bounds,width,height,values,noData,fileName:file.name};terrainGroup.add(mesh);terrainGroup.position.z=terrainVerticalOffset;terrainGroup.visible=true;$('#terrainVisible').checked=true;$('#demStatus').textContent=`${file.name} · ${width}×${height} mesh`;if(!state.layers.length&&geometry.boundingBox){const viewBox=geometry.boundingBox.clone();viewBox.min.z+=terrainVerticalOffset;viewBox.max.z+=terrainVerticalOffset;fitBox(viewBox)}toast(`สร้าง Terrain จาก ${file.name} สำเร็จ · ${width*height} vertices${hadImagery||hadRegional?' · กรุณาโหลดพื้นหลังใหม่':''}`);invalidate();
}
...
function syncOsmOffsets(){osmGroundGroup.position.z=terrainVerticalOffset;osmBuildingsGroup.position.z=terrainVerticalOffset+osmBuildingsOffset;osmRoadsGroup.position.z=terrainVerticalOffset+osmRoadsOffset}
function syncImageryOffset(){imageryGroup.position.z=(terrainData?terrainVerticalOffset:0)+imageryVerticalOffset}
function applyTerrainOffset(value,source='manual'){if(!Number.isFinite(value)){toast('กรุณาระบุ Vertical Offset เป็นตัวเลข',true);return}const delta=value-terrainVerticalOffset;terrainVerticalOffset=value;terrainGroup.position.z=value;syncImageryOffset();regionalTerrainGroup.traverse(o=>{const position=o.geometry?.attributes?.position,weight=o.geometry?.attributes?.regionalWeight;if(!position||!weight)return;for(let i=0;i<position.count;i++)position.setZ(i,position.getZ(i)+delta*(1-weight.getX(i)));position.needsUpdate=true;o.geometry.computeVertexNormals();o.geometry.computeBoundingBox()});syncOsmOffsets();$('#demVerticalOffset').value=value.toFixed(3);$('#verticalStatus').textContent=`Corrected Z = DEM Z ${value<0?'−':'+'} ${Math.abs(value).toFixed(3)} m${source==='control'?' · calibrated from control':''}`;toast(`ปรับระดับ Project DEM ${value>=0?'+':''}${value.toFixed(3)} m สำเร็จ`);invalidate()}
$('#applyVerticalOffset').addEventListener('click',()=>{const input=$('#demVerticalOffset');applyTerrainOffset(input.value.trim()===''?NaN:Number(input.value))});$('#calibrateTerrain').addEventListener('click',()=>{const input=$('#controlElevation'),elevation=input.value.trim()===''?NaN:Number(input.value);if(!terrainData){toast('กรุณาโหลด DEM ก่อนปรับระดับ',true);return}if(!Number.isFinite(elevation)){toast('กรุณากรอกระดับจริงของ Control Point',true);return}if(state.measuring)toggleMeasure(false);state.pivotMode=false;state.calibratingTerrain=true;renderer.domElement.classList.add('terrain-calibrating');toast('คลิกตำแหน่ง Control Point บนผิว DEM')});
...
function imageryTileGeometry(sw,ne){const segments=terrainData?24:1,positions=[],uvs=[],indices=[],valid=[];for(let row=0;row<=segments;row++)for(let col=0;col<=segments;col++){const u=col/segments,v=1-row/segments,x=THREE.MathUtils.lerp(sw.x,ne.x,u),y=THREE.MathUtils.lerp(ne.y,sw.y,row/segments),z=terrainData?projectBaseHeightAt(x,y):-.12;valid.push(z!=null);positions.push(x,y,z==null?0:z+.04);uvs.push(u,v)}for(let row=0;row<segments;row++)for(let col=0;col<segments;col++){const a=row*(segments+1)+col,b=a+1,c=a+segments+1,d=c+1;if(valid[a]&&valid[b]&&valid[c])indices.push(a,c,b);if(valid[b]&&valid[c]&&valid[d])indices.push(b,c,d)}const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.setAttribute('uv',new THREE.Float32BufferAttribute(uvs,2));geometry.setIndex(indices);geometry.computeVertexNormals();return geometry}
function clearRegionalTerrain(){regionalTerrainGroup.traverse(o=>{o.geometry?.dispose();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>{m.map?.dispose();m.dispose()})});regionalTerrainGroup.clear();regionalTerrainGroup.position.z=0;$('#regionalStatus').textContent='Mapzen Terrain + Esri Imagery · ยังไม่ได้โหลด';invalidate()}
async function loadImageBitmap(url){const response=await fetch(url);if(!response.ok)throw new Error(`HTTP ${response.status}`);return createImageBitmap(await response.blob())}
function decodeTerrarium(bitmap){const canvas=document.createElement('canvas');canvas.width=bitmap.width;canvas.height=bitmap.height;const context=canvas.getContext('2d',{willReadFrequently:true});context.drawImage(bitmap,0,0);return context.getImageData(0,0,bitmap.width,bitmap.height).data}
function projectBaseHeightAt(x,y){const corrected=terrainHeightAt(x,y);return corrected==null?null:corrected-terrainVerticalOffset}
function blendRegionalHeight(x,y,regionalBase){const project=projectBaseHeightAt(x,y);if(project!=null)return{z:project-.08,weight:0};if(!terrainData)return{z:regionalBase,weight:1};const b=terrainData.bounds,cx=THREE.MathUtils.clamp(x,b.minX,b.maxX),cy=THREE.MathUtils.clamp(y,b.minY,b.maxY),edge=projectBaseHeightAt(cx,cy),distance=Math.hypot(x-cx,y-cy),blend=200;if(edge!=null&&distance<blend){const weight=distance/blend;return{z:THREE.MathUtils.lerp(edge-.08,regionalBase,weight),weight}}return{z:regionalBase,weight:1}}
function regionalTileGeometry(tile,zoom,pixels,radius){const segments=32,b=tileBounds(tile.x,tile.y,zoom),positions=[],uvs=[],weights=[],indices=[],inside=[];for(let row=0;row<=segments;row++)for(let col=0;col<=segments;col++){const u=col/segments,v=1-row/segments,lon=THREE.MathUtils.lerp(b.west,b.east,u),lat=THREE.MathUtils.lerp(b.north,b.south,row/segments),p=latLonToLocal(lat,lon),px=Math.min(255,Math.round(u*255)),py=Math.min(255,Math.round(row/segments*255)),i=(py*256+px)*4,elevation=pixels[i]*256+pixels[i+1]+pixels[i+2]/256-32768,blended=blendRegionalHeight(p.x,p.y,elevation-state.origin.z);positions.push(p.x,p.y,blended.z+terrainVerticalOffset*(1-blended.weight)+regionalVerticalOffset*blended.weight);uvs.push(u,v);weights.push(blended.weight);inside.push(Math.hypot(p.x,p.y)<=radius*1.12)}for(let row=0;row<segments;row++)for(let col=0;col<segments;col++){const a=row*(segments+1)+col,b=a+1,c=a+segments+1,d=c+1;if(inside[a]||inside[b]||inside[c])indices.push(a,c,b);if(inside[b]||inside[c]||inside[d])indices.push(b,c,d)}const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.setAttribute('uv',new THREE.Float32BufferAttribute(uvs,2));geometry.setAttribute('regionalWeight',new THREE.Float32BufferAttribute(weights,1));geometry.setIndex(indices);geometry.computeVertexNormals();return geometry}
function applyRegionalOffset(value){if(!Number.isFinite(value)){toast('กรุณาระบุ Regional Terrain Offset เป็นตัวเลข',true);return}const delta=value-regionalVerticalOffset;regionalVerticalOffset=value;regionalTerrainGroup.traverse(o=>{const position=o.geometry?.attributes?.position,weight=o.geometry?.attributes?.regionalWeight;if(!position||!weight)return;for(let i=0;i<position.count;i++)position.setZ(i,position.getZ(i)+delta*weight.getX(i));position.needsUpdate=true;o.geometry.computeVertexNormals();o.geometry.computeBoundingBox()});$('#regionalOffset').value=value.toFixed(3);$('#regionalOffsetStatus').textContent=`Offset ${value>=0?'+':''}${value.toFixed(3)} m · Blend ที่ขอบ Project DEM`;toast(`ปรับ Regional Terrain ${value>=0?'+':''}${value.toFixed(3)} m สำเร็จ`);invalidate()}
async function loadRegionalTerrain(){const radius=+$('#regionalRadius').value,zoom=radius<=2000?13:radius<=5000?12:11,center=projectToLatLon(state.origin.x,state.origin.y),latRadius=THREE.MathUtils.radToDeg(radius/6378137),lonRadius=latRadius/Math.max(Math.cos(THREE.MathUtils.degToRad(center.lat)),.1),nw=tileXY(center.lat+latRadius,center.lon-lonRadius,zoom),se=tileXY(center.lat-latRadius,center.lon+lonRadius,zoom),tiles=[];for(let y=nw.y;y<=se.y;y++)for(let x=nw.x;x<=se.x;x++)tiles.push({x,y});if(tiles.length>36){toast('พื้นที่ Regional Terrain มี Tile มากเกินไป',true);return}const button=$('#loadRegionalTerrain'),next=new THREE.Group();button.disabled=true;button.textContent=`กำลังโหลด 0/${tiles.length}…`;let loaded=0;try{for(const tile of tiles){try{const [heightBitmap,texture]=await Promise.all([loadImageBitmap(`https://s3.amazonaws.com/elevation-tiles-prod/terrarium/${zoom}/${tile.x}/${tile.y}.png`),new THREE.TextureLoader().loadAsync(`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${zoom}/${tile.y}/${tile.x}`)]),pixels=decodeTerrarium(heightBitmap),geometry=regionalTileGeometry(tile,zoom,pixels,radius),material=new THREE.MeshBasicMaterial({map:texture,side:THREE.DoubleSide,depthWrite:true,fog:false}),mesh=new THREE.Mesh(geometry,material);heightBitmap.close();texture.colorSpace=THREE.SRGBColorSpace;mesh.userData.contextOnly=true;mesh.renderOrder=-10;next.add(mesh);loaded++;button.textContent=`กำลังโหลด ${loaded}/${tiles.length}…`}catch(error){console.warn('Regional terrain tile failed',tile,error)}}if(!loaded)throw new Error('ไม่สามารถโหลด Terrain Tiles ได้');clearRegionalTerrain();while(next.children.length)regionalTerrainGroup.add(next.children[0]);regionalTerrainGroup.position.z=0;regionalTerrainGroup.visible=true;$('#regionalVisible').checked=true;$('#regionalStatus').textContent=`รัศมี ${(radius/1000).toFixed(0)} km · ${loaded}/${tiles.length} tiles · zoom ${zoom}`;toast(`โหลดภูมิประเทศรอบโครงการสำเร็จ · ${loaded}/${tiles.length} tiles`);if(!state.layers.length&&!terrainData)fitBox(new THREE.Box3().setFromObject(regionalTerrainGroup));invalidate()}catch(error){next.traverse(o=>{o.geometry?.dispose();o.material?.map?.dispose();o.material?.dispose()});toast(`โหลด Regional Terrain ไม่สำเร็จ: ${error.message}`,true)}finally{button.disabled=false;button.textContent='โหลด Terrain รอบโครงการ'}}
$('#loadRegionalTerrain').addEventListener('click',loadRegionalTerrain);$('#regionalVisible').addEventListener('change',e=>{regionalTerrainGroup.visible=e.target.checked;invalidate()});$('#applyRegionalOffset').addEventListener('click',()=>applyRegionalOffset(Number($('#regionalOffset').value)));
function clearImagery(){imageryGroup.traverse(o=>{o.geometry?.dispose();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>{m.map?.dispose();m.dispose()})});imageryGroup.clear();syncImageryOffset();invalidate()}
function applyImageryOffset(value){if(!Number.isFinite(value)){toast('กรุณาระบุ Esri Imagery Offset เป็นตัวเลข',true);return}imageryVerticalOffset=value;syncImageryOffset();$('#imageryOffset').value=value.toFixed(3);$('#imageryOffsetStatus').textContent=`Offset ${value>=0?'+':''}${value.toFixed(3)} m · อ้างอิงผิว Project DEM`;toast(`ปรับภาพโครงการ Esri ${value>=0?'+':''}${value.toFixed(3)} m สำเร็จ`);invalidate()}
async function loadEsriImagery(){
  const radius=+$('#imageryRadius').value,zoom=+$('#imageryZoom').value,center=projectToLatLon(state.origin.x,state.origin.y),provider=$('#imageryProvider').value,key=$('#mapTilerKey').value.trim(),providerLabel=provider==='maptiler'?'MapTiler Satellite':'Esri World Imagery';if(!Number.isFinite(center.lat)||Math.abs(center.lat)>85){toast('Local Origin หรือระบบพิกัดไม่ถูกต้อง',true);return}if(provider==='maptiler'&&!key){toast('กรุณากรอก MapTiler Public API Key',true);return}
  ...
  try {for(const tile of tiles){try{const url=provider==='maptiler'?`https://api.maptiler.com/tiles/satellite-v2/${zoom}/${tile.x}/${tile.y}.jpg?key=${encodeURIComponent(key)}`:`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${zoom}/${tile.y}/${tile.x}`,texture=await loader.loadAsync(url);texture.colorSpace=THREE.SRGBColorSpace;const b=tileBounds(tile.x,tile.y,zoom),sw=latLonToLocal(b.south,b.west),ne=latLonToLocal(b.north,b.east),geometry=imageryTileGeometry(sw,ne),material=new THREE.MeshBasicMaterial({map:texture,side:THREE.DoubleSide,transparent:true,opacity:.92,depthWrite:false}),mesh=new THREE.Mesh(geometry,material);mesh.renderOrder=-5;mesh.userData.contextOnly=true;next.add(mesh);loaded++}catch(error){console.warn('Imagery tile failed',tile,error)}}if(!loaded)throw new Error('ไม่สามารถอ่าน Tile ภาพดาวเทียมได้');clearImagery();while(next.children.length)imageryGroup.add(next.children[0]);syncImageryOffset();imageryGroup.visible=true;$('#imageryVisible').checked=true;$('#imageryCredit').textContent=provider==='maptiler'?'Imagery © MapTiler and data providers':'Imagery © Esri and data providers';toast(`โหลด ${providerLabel} สำเร็จ · zoom ${zoom} · ${loaded}/${tiles.length} tiles${terrainData?' · ตัดตามขอบ DEM':''}`);invalidate()}catch(error){next.traverse(o=>{o.geometry?.dispose();o.material?.map?.dispose();o.material?.dispose()});console.error(error);toast(`โหลดภาพดาวเทียมไม่สำเร็จ: ${error.message}`,true)}finally{$('#loadImagery').disabled=false;$('#loadImagery').textContent='โหลดภาพโครงการ'}
}
function updateImagerySettings(){const provider=$('#imageryProvider').value,zoom=+$('#imageryZoom').value,center=projectToLatLon(state.origin.x,state.origin.y),metresPerPixel=156543.03392*Math.cos(THREE.MathUtils.degToRad(Number.isFinite(center.lat)?center.lat:0))/2**zoom;$('#mapTilerKeyField').hidden=provider!=='maptiler';$('#imageryResolutionHint').textContent=`Zoom ${zoom} · ประมาณ ${metresPerPixel.toFixed(metresPerPixel<1?2:1)} m/px ณ ตำแหน่งโครงการ · ความคมจริงขึ้นกับภาพต้นทาง`;$('#imageryCredit').textContent=provider==='maptiler'?'Imagery © MapTiler and data providers':'Imagery © Esri and data providers'}
$('#loadImagery').addEventListener('click',loadEsriImagery);$('#imageryVisible').addEventListener('change',e=>{imageryGroup.visible=e.target.checked;invalidate()});$('#applyImageryOffset').addEventListener('click',()=>applyImageryOffset(Number($('#imageryOffset').value)));$('#imageryProvider').addEventListener('change',updateImagerySettings);$('#imageryZoom').addEventListener('change',updateImagerySettings);updateImagerySettings();
function disposeGroupChildren(group){group.traverse(o=>{if(o===group)return;o.geometry?.dispose();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>m.dispose())});group.clear()}
function clearBackground(){disposeGroupChildren(osmGroundGroup);disposeGroupChildren(osmBuildingsGroup);disposeGroupChildren(osmRoadsGroup);backgroundGroup.position.set(0,0,0);syncOsmOffsets();invalidate()}
...
function addOsmBuilding(element,center){const nodes=element.geometry;if(!nodes?.length)return;const points=nodes.map(n=>latLonToLocal(n.lat,n.lon,center));if(points.length>2&&points[0].distanceTo(points.at(-1))<.01)points.pop();if(points.length<3)return;const shape=new THREE.Shape(points),height=osmHeight(element.tags||{}),geometry=new THREE.ExtrudeGeometry(shape,{depth:height,bevelEnabled:false,steps:1}),material=new THREE.MeshStandardMaterial({color:osmColor(element.tags||{}),roughness:.92,metalness:0,transparent:true,opacity:.82}),centerPoint=points.reduce((sum,p)=>sum.add(p),new THREE.Vector2()).multiplyScalar(1/points.length),mesh=new THREE.Mesh(geometry,material);mesh.position.z=(projectBaseHeightAt(centerPoint.x,centerPoint.y)??0)+.05;osmBuildingsGroup.add(mesh)}
function addOsmRoad(element,center){const nodes=element.geometry;if(!nodes?.length)return;const points=nodes.map(n=>{const p=latLonToLocal(n.lat,n.lon,center);return new THREE.Vector3(p.x,p.y,(projectBaseHeightAt(p.x,p.y)??0)+.08)});if(points.length<2)return;const geometry=new THREE.BufferGeometry().setFromPoints(points),material=new THREE.LineBasicMaterial({color:0x65716e,transparent:true,opacity:.75});osmRoadsGroup.add(new THREE.Line(geometry,material))}
function applyOsmOffset(kind,value){if(!Number.isFinite(value)){toast('กรุณาระบุ OSM Offset เป็นตัวเลข',true);return}if(kind==='buildings'){osmBuildingsOffset=value;$('#osmBuildingsOffset').value=value.toFixed(3);$('#osmBuildingsOffsetStatus').textContent=`Offset ${value>=0?'+':''}${value.toFixed(3)} m`}else{osmRoadsOffset=value;$('#osmRoadsOffset').value=value.toFixed(3);$('#osmRoadsOffsetStatus').textContent=`Offset ${value>=0?'+':''}${value.toFixed(3)} m`}syncOsmOffsets();toast(`ปรับ OSM ${kind==='buildings'?'Buildings':'Roads'} ${value>=0?'+':''}${value.toFixed(3)} m สำเร็จ`);invalidate()}
async function loadOsmBackground(){
  ...
  try {const query=`[out:json][timeout:25];(way[building](around:${radius},${center.lat},${center.lon});way[highway](around:${radius},${center.lat},${center.lon}););out geom;`,response=await fetch('https://overpass-api.de/api/interpreter',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:`data=${encodeURIComponent(query)}`});if(!response.ok)throw new Error(`Overpass HTTP ${response.status}`);const data=await response.json();clearBackground();if(!terrainData){const ground=new THREE.Mesh(new THREE.CircleGeometry(radius,64),new THREE.MeshStandardMaterial({color:0x172723,roughness:1,transparent:true,opacity:.78,side:THREE.DoubleSide}));ground.position.z=-.05;osmGroundGroup.add(ground)}let buildings=0,roads=0;for(const element of data.elements||[]){if(element.tags?.building){addOsmBuilding(element,center);buildings++}else if(element.tags?.highway){addOsmRoad(element,center);roads++}}syncOsmOffsets();backgroundGroup.visible=true;$('#backgroundVisible').checked=true;if(!buildings&&!roads)toast(`ไม่พบข้อมูล OSM บริเวณนี้ · ตรวจ ${selectedCrs().label} และ Local Origin`,true);else toast(`โหลดพื้นหลัง OSM สำเร็จ · อาคาร ${buildings} · ถนน ${roads}${terrainData?' · วางบน Terrain แล้ว':''}`);invalidate()}catch(error){console.error(error);toast(`โหลดพื้นหลังไม่สำเร็จ: ${error.message}`,true)}finally{$('#loadBackground').disabled=false;$('#loadBackground').textContent='โหลดพื้นหลัง 3D'}
}
$('#loadBackground').addEventListener('click',loadOsmBackground);$('#backgroundVisible').addEventListener('change',e=>{backgroundGroup.visible=e.target.checked;invalidate()});$('#applyOsmBuildingsOffset').addEventListener('click',()=>applyOsmOffset('buildings',Number($('#osmBuildingsOffset').value)));$('#applyOsmRoadsOffset').addEventListener('click',()=>applyOsmOffset('roads',Number($('#osmRoadsOffset').value)));document.querySelectorAll('[data-offset-target]').forEach(button=>button.addEventListener('click',()=>{const target=button.dataset.offsetTarget,step=Number(button.dataset.offsetStep);if(target==='regional')applyRegionalOffset(regionalVerticalOffset+step);else if(target==='imagery')applyImageryOffset(imageryVerticalOffset+step);else applyOsmOffset(target,(target==='buildings'?osmBuildingsOffset:osmRoadsOffset)+step)}));syncImageryOffset();syncOsmOffsets();
