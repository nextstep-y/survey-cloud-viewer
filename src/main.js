...
async function loadDem(file){
  ...
  const zone=Math.round(+$('#utmZone').value),hemisphere=$('#utmHemisphere').value,epsg=Number(geoKeys.ProjectedCSTypeGeoKey),expectedEpsg=(hemisphere==='S'?32700:32600)+zone;if(projected&&Number.isFinite(epsg)&&epsg!==32767&&epsg!==expectedEpsg)throw new Error(`DEM EPSG:${epsg} ไม่ตรงกับ UTM Zone ${zone}${hemisphere} (EPSG:${expectedEpsg})`);
  if(projected)bounds={minX:bbox[0]-state.origin.x,minY:bbox[1]-state.origin.y,maxX:bbox[2]-state.origin.x,maxY:bbox[3]-state.origin.y};else{const center=utmToLatLon(state.origin.x,state.origin.y,zone,hemisphere),sw=latLonToLocal(bbox[1],bbox[0],center),ne=latLonToLocal(bbox[3],bbox[2],center);bounds={minX:sw.x,minY:sw.y,maxX:ne.x,maxY:ne.y}}
