...
const state = { layers: [], selectedId: null, selectedEntity:null, origin: new THREE.Vector3(), nextId: 1, measuring:false, measureStart:null, measurePreview:null, measureAxis:'auto', pivotMode:false };
...
$('#applyOrigin').addEventListener('click',()=>{state.origin.set(+$(' #originX'.trim()).value||0,+$('#originY').value||0,+$('#originZ').value||0);state.layers.forEach(updateLayerTransform);toast('อัปเดต Local Origin แล้ว');fitAll();});
...
// Measurement is raycast only on click—not on every pointer move—so a million
// displayed points remain responsive. Values are converted back to world CRS.
...
function pickModelPoint(e){
  ...
  return raycaster.ray.intersectPlane(ground,new THREE.Vector3())?.clone()||null;
}
...
function choosePivotAt(e){const point=pickModelPoint(e);if(!point)return;setNavigationPivot(point);state.pivotMode=false;$('#pivotBtn').classList.remove('active');renderer.domElement.classList.remove('pivoting')}
...
function clearMeasureGraphics(){measureGroup.traverse(o=>{o.geometry?.dispose();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>m.dispose())});measureGroup.clear();state.measurePreview=null}
function clearMeasure(){state.measureStart=null;clearMeasureGraphics();['#deltaX','#deltaY','#deltaZ','#distance2d','#distance3d'].forEach(s=>$(s).textContent='—');$('#measureHint').textContent='คลิกจุดเริ่มต้นบนโมเดล';invalidate()}
...
function snappedEnd(start,end){
  ...
  if(axis==='auto'){const a=[Math.abs(delta.x),Math.abs(delta.y),Math.abs(delta.z)];axis=['x','y','z'][a.indexOf(Math.max(...a))]}
...
let previewTimer=0,lastPreviewEvent=null;renderer.domElement.addEventListener('pointermove',e=>{if(!state.measuring||!state.measureStart)return;lastPreviewEvent={clientX:e.clientX,clientY:e.clientY};if(previewTimer)return;previewTimer=setTimeout(()=>{previewTimer=0;if(!state.measureStart||!lastPreviewEvent)return;const point=pickModelPoint(lastPreviewEvent);if(point)updateMeasurePreview(point)},90)});
renderer.domElement.addEventListener('dblclick',e=>{if(!state.measuring)choosePivotAt(e)});
renderer.domElement.addEventListener('pointerdown',e=>{pointerDown={x:e.clientX,y:e.clientY}});renderer.domElement.addEventListener('pointerup',e=>{if(!pointerDown||Math.hypot(e.clientX-pointerDown.x,e.clientY-pointerDown.y)>4)return;if(state.pivotMode){choosePivotAt(e);return}if(!state.measuring){selectDxfAt(e);return}const point=pickModelPoint(e);if(!point)return;if(!state.measureStart){state.measureStart=point;clearMeasureGraphics();measureGroup.add(marker(point,0xffffff));$('#measureHint').textContent=`จุดเริ่ม: E ${fmt(point.x+state.origin.x)} · N ${fmt(point.y+state.origin.y)} · Z ${fmt(point.z+state.origin.z)} · เลื่อนเมาส์ดูแนว แล้วคลิกจุดปลาย`;invalidate()}else finishMeasure(point)});
