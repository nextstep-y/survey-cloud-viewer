  ...
  <div class="shell">
    ...
    <main>
      <aside class="sidebar">
        ...
        <div class="drop-zone" id="dropZone"><div class="upload-icon">↥</div><b>วางไฟล์ที่นี่</b><span>หรือคลิกเพื่อเลือกไฟล์</span><small>LAS · LAZ · PLY · GLB · GLTF · OBJ · STL · DXF</small></div>
        <input id="fileInput" type="file" multiple accept=".las,.laz,.ply,.glb,.gltf,.obj,.stl,.dxf" hidden />
        ...
        <section class="coordinate-card">
          ...
          <div class="background-settings"><div class="card-title"><span>▧</span><div><b>พื้นหลังภูมิศาสตร์</b><small>ESRI IMAGERY + OSM 3D</small></div></div><div class="background-grid"><label>UTM Zone<input id="utmZone" type="number" min="1" max="60" value="47"></label><label>Hemisphere<select id="utmHemisphere"><option value="N" selected>North</option><option value="S">South</option></select></label><label>รัศมี<select id="osmRadius"><option value="300">300 m</option><option value="600" selected>600 m</option><option value="1000">1,000 m</option></select></label><label>ความละเอียด<select id="imageryZoom"><option value="15">ต่ำ · เร็ว</option><option value="16" selected>กลาง</option><option value="17">สูง</option></select></label></div><button class="secondary imagery-button" id="loadImagery">โหลดภาพดาวเทียม Esri</button><label class="toggle-row background-toggle">แสดงภาพดาวเทียม<input id="imageryVisible" type="checkbox" checked><i></i></label><button class="secondary" id="loadBackground">โหลดอาคารและถนน OSM</button><label class="toggle-row background-toggle">แสดง OSM 3D<input id="backgroundVisible" type="checkbox" checked><i></i></label><small class="osm-credit">Imagery © Esri and data providers · <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap contributors</a></small></div>
...
function latLonToLocal(lat,lon,center){const earth=6378137,latRad=THREE.MathUtils.degToRad(center.lat);return new THREE.Vector2(earth*THREE.MathUtils.degToRad(lon-center.lon)*Math.cos(latRad),earth*THREE.MathUtils.degToRad(lat-center.lat))}
...
async function loadEsriImagery(){
  const zone=Math.round(+$('#utmZone').value),hemisphere=$('#utmHemisphere').value,radius=+$('#osmRadius').value,zoom=+$('#imageryZoom').value;if(zone<1||zone>60){toast('กรุณาระบุ UTM Zone 1–60',true);return}
  const center=utmToLatLon(state.origin.x,state.origin.y,zone,hemisphere);if(!Number.isFinite(center.lat)||Math.abs(center.lat)>85){toast('Local Origin หรือ UTM Zone ไม่ถูกต้อง',true);return}
  ...
  try {for(const tile of tiles){try{const texture=await loader.loadAsync(`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${zoom}/${tile.y}/${tile.x}`);texture.colorSpace=THREE.SRGBColorSpace;const b=tileBounds(tile.x,tile.y,zoom),sw=latLonToLocal(b.south,b.west,center),ne=latLonToLocal(b.north,b.east,center),geometry=new THREE.PlaneGeometry(ne.x-sw.x,ne.y-sw.y),material=new THREE.MeshBasicMaterial({map:texture,side:THREE.DoubleSide,transparent:true,opacity:.92,depthWrite:false}),mesh=new THREE.Mesh(geometry,material);mesh.position.set((sw.x+ne.x)/2,(sw.y+ne.y)/2,-.12);mesh.renderOrder=-5;next.add(mesh);loaded++}catch(error){console.warn('Imagery tile failed',tile,error)}}if(!loaded)throw new Error('ไม่สามารถอ่าน Tile ภาพดาวเทียมได้');clearImagery();while(next.children.length)imageryGroup.add(next.children[0]);imageryGroup.visible=true;$('#imageryVisible').checked=true;toast(`โหลด Esri World Imagery สำเร็จ · ${loaded}/${tiles.length} tiles`);invalidate()}catch(error){next.traverse(o=>{o.geometry?.dispose();o.material?.map?.dispose();o.material?.dispose()});console.error(error);toast(`โหลดภาพดาวเทียมไม่สำเร็จ: ${error.message}`,true)}finally{$('#loadImagery').disabled=false;$('#loadImagery').textContent='โหลดภาพดาวเทียม Esri'}
}
