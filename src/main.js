...
async function loadDem(file){
  ...
  if(!(bounds.maxX>bounds.minX&&bounds.maxY>bounds.minY))throw new Error('ขอบเขตพิกัด GeoTIFF ไม่ถูกต้อง');const noDataRaw=image.getGDALNoData(),parsedNoData=noDataRaw==null?null:Number(noDataRaw),noData=Number.isFinite(parsedNoData)?parsedNoData:null,geometry=makeTerrainGeometry(bounds,width,height,values,noData),material=new THREE.MeshStandardMaterial({color:0x49655c,roughness:1,metalness:0,side:THREE.DoubleSide,transparent:true,opacity:.9}),mesh=new THREE.Mesh(geometry,material),hadImagery=imageryGroup.children.length>0;mesh.userData.contextOnly=true;clearTerrain();if(hadImagery)clearImagery();terrainData={bounds,width,height,values,noData,fileName:file.name};terrainGroup.add(mesh);terrainGroup.position.z=terrainVerticalOffset;terrainGroup.visible=true;$('#terrainVisible').checked=true;$('#demStatus').textContent=`${file.name} · ${width}×${height} mesh`;if(!state.layers.length&&geometry.boundingBox){const viewBox=geometry.boundingBox.clone();viewBox.min.z+=terrainVerticalOffset;viewBox.max.z+=terrainVerticalOffset;fitBox(viewBox)}toast(`สร้าง Terrain จาก ${file.name} สำเร็จ · ${width*height} vertices${hadImagery?' · กรุณาโหลดภาพดาวเทียมใหม่':''}`);invalidate();
}
