...
let terrainData=null,terrainVerticalOffset=0;
...
function applyTerrainOffset(value,source='manual'){if(!Number.isFinite(value)){toast('กรุณาระบุ Vertical Offset เป็นตัวเลข',true);return}const delta=value-terrainVerticalOffset;terrainVerticalOffset=value;terrainGroup.position.z=value;regionalTerrainGroup.position.z=value;imageryGroup.position.z+=delta;backgroundGroup.position.z+=delta;$('#demVerticalOffset').value=value.toFixed(3);$('#verticalStatus').textContent=`Corrected Z = DEM Z ${value<0?'−':'+'} ${Math.abs(value).toFixed(3)} m${source==='control'?' · calibrated from control':''}`;toast(`ปรับระดับพื้นหลัง ${value>=0?'+':''}${value.toFixed(3)} m สำเร็จ`);invalidate()}
...
function clearBackground(){backgroundGroup.traverse(o=>{o.geometry?.dispose();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>m.dispose())});backgroundGroup.clear();backgroundGroup.position.z=0;invalidate()}
