    ...
    <main>
      <aside class="sidebar">
        ...
        <section class="coordinate-card">
          ...
          <div class="background-settings"><div class="card-title"><span>▧</span><div><b>พื้นหลัง 3D ฟรี</b><small>OPENSTREETMAP BUILDINGS + ROADS</small></div></div><div class="background-grid"><label>UTM Zone<input id="utmZone" type="number" min="1" max="60" value="47"></label><label>Hemisphere<select id="utmHemisphere"><option value="N" selected>North</option><option value="S">South</option></select></label><label>รัศมี<select id="osmRadius"><option value="300">300 m</option><option value="600" selected>600 m</option><option value="1000">1,000 m</option></select></label></div><button class="secondary" id="loadBackground">โหลดพื้นหลัง 3D</button><label class="toggle-row background-toggle">แสดงพื้นหลัง<input id="backgroundVisible" type="checkbox" checked><i></i></label><small class="osm-credit"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap contributors</a> · ODbL</small></div>
...
$('#applyOrigin').addEventListener('click',()=>{state.origin.set(+$(' #originX'.trim()).value||0,+$('#originY').value||0,+$('#originZ').value||0);state.layers.forEach(updateLayerTransform);if(backgroundGroup.children.length){clearBackground();toast('อัปเดต Local Origin แล้ว · กรุณาโหลดพื้นหลัง 3D ใหม่')}else toast('อัปเดต Local Origin แล้ว');fitAll();});
