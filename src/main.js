...
async function loadEsriImagery(){
  ...
  try {for(const tile of tiles){try{const texture=await loader.loadAsync(`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${zoom}/${tile.y}/${tile.x}`);texture.colorSpace=THREE.SRGBColorSpace;const b=tileBounds(tile.x,tile.y,zoom),sw=latLonToLocal(b.south,b.west),ne=latLonToLocal(b.north,b.east),geometry=imageryTileGeometry(sw,ne),material=new THREE.MeshBasicMaterial({map:texture,side:THREE.DoubleSide,transparent:true,opacity:.92,depthWrite:false}),mesh=new THREE.Mesh(geometry,material);mesh.renderOrder=-5;mesh.userData.contextOnly=true;next.add(mesh);loaded++}catch(error){console.warn('Imagery tile failed',tile,error)}}if(!loaded)throw new Error('ไม่สามารถอ่าน Tile ภาพดาวเทียมได้');clearImagery();while(next.children.length)imageryGroup.add(next.children[0]);syncImageryOffset();imageryGroup.visible=true;$('#imageryVisible').checked=true;toast(`โหลด Esri World Imagery สำเร็จ · ${loaded}/${tiles.length} tiles${terrainData?' · ตัดตามขอบ DEM':''}`);invalidate()}catch(error){next.traverse(o=>{o.geometry?.dispose();o.material?.map?.dispose();o.material?.dispose()});console.error(error);toast(`โหลดภาพดาวเทียมไม่สำเร็จ: ${error.message}`,true)}finally{$('#loadImagery').disabled=false;$('#loadImagery').textContent='โหลดภาพโครงการ Esri'}
}
$('#loadImagery').addEventListener('click',loadEsriImagery);$('#imageryVisible').addEventListener('change',e=>{imageryGroup.visible=e.target.checked;invalidate()});$('#applyImageryOffset').addEventListener('click',()=>applyImageryOffset(Number($('#imageryOffset').value)));
